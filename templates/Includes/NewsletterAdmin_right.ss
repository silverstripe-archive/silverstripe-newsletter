...
<form class="actionparams" id="action_send_options" style="display:none" action="admin/newsletter/sendnewsletter">
	...
	<p class="actions">
		<ul class="optionset">
			<li class="cancel action"><input type="submit" id="action_send_cancel" value="<% _t('CANCEL', 'Cancel') %>" /></li>
			<li class="submit action"><input type="submit" value="<% _t('SEND', 'Send newsletter') %>" /></li>
		</ul>
	</p>
</form>
...
<% if EditForm %>
	$EditForm
<% else %>
	<form id="Form_EditForm" action="admin/newsletter/EditForm" method="post" enctype="multipart/form-data">
